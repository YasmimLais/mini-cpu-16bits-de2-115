module CPU (
    ...
    always@(posedge clk) begin
        ...
        else begin // <--- ADICIONE ESTE ELSE AQUI
            case(estado)
                S_DESLIGADO: begin
                    estado <= S_AGUARDANDO;
                end
                ...
                S_AGUARDANDO: begin
                    if (flag_enviar) begin
                        ...
                        lcd_logger_latched <= escrita_endereco;

                        if (opcode_chaves == OP_DISPLAY)
                            lcd_result_latched <= operador_1;
                        else
                            lcd_result_latched <= conteudo_resultado;
